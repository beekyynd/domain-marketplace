<!-- Footer -->

<section class="footer" id="footer">

<div class="container">

<div class="row p-3">

<!-- Contact -->

<!-- Sellers -->

<div class="col-lg-3 col-md-4 col-sm-12">

<h5 class="c-us">Sellers</h5>

<ul class="c-list">

<li><a href="#" class="text-muted">Create an account</a></li>

<li><a href="#" class="text-muted">Login</a></li>

</ul>

</div>

<!-- Domainer -->

<div class="col-lg-3 col-md-4 col-sm-12">

<h5 class="c-us">Domain Marketplace</h5>

<ul class="c-list">

<li><a href="#" class="text-muted">About us</a></li>

<li><a href="#" class="text-muted">Contact us</a></li>

<li><a href="#" class="text-muted">FAQs</a></li>

</ul>

</div>

<!-- Legal -->

<div class="col-lg-3 col-md-4 col-sm-12">

<h5 class="c-us">Legal</h5>

<ul class="c-list">

<li><a href="#" class="text-muted">Refund policy</a></li>

<li><a href="#" class="text-muted">Terms of Use</a></li>

<li><a href="#" class="text-muted">Sellers Terms of Use</a></li>

<li><a href="#" class="text-muted">Buyers Terms of Use</a></li>

<li><a href="#" class="text-muted">Privacy Policy</a></li>

</ul>

</div>

<!-- End Footer -->

</div>

<button onclick="topButton()" class="btn btn-flat f-rad p-2" id="myBtn"><i class="fas fa-arrow-up"></i></button>

<hr class="h-r mb-5"></hr>

<div class="row">

<div class="col-lg-12 col-md-6 col-sm-12 text-center">


<h6 class="text-dark">&copy; 2024 Domainer Marketplace. All rights reserved</h6>

<span class="f-icons">

    <i class="fa-brands fa-x-twitter"></i>
    <i class="fa-brands fa-linkedin-in"></i>
    <i class="fa-brands fa-instagram"></i>
    
    </span>    


</div>

</div>

</div>

<style>

.f-rad {

width: 50px;

height: 50px;

border-radius: 50% !important;
}


</style>

</section>

</body>

</html>